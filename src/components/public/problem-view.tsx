...
export function ProblemView({ problem }: { problem: Problem }) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 pb-4 border-b border-zinc-800">
        ...
        {problem.sourceUrl && (
          <a
            ...
            className="inline-flex items-center gap-1.5 text-xs font-medium text-zinc-300 hover:text-white px-3 py-1.5 rounded-md bg-zinc-900 hover:bg-zinc-800 border border-zinc-800 transition-colors w-fit"
          >
      ...
      <section className="rounded-xl border border-zinc-800/80 bg-zinc-900/30 p-4 sm:p-5 space-y-4">
        <div>
          ...
          <div className="space-y-3 pt-2">
            ...
            <div className="grid gap-2.5">
              {problem.examples.map((ex, i) => (
                <div
                  key={i}
                  className="rounded-lg border border-zinc-800 bg-zinc-950/60 p-3 text-xs font-mono space-y-1"
                >
                  <div className="text-zinc-300">
                    <span className="text-zinc-400">Input:</span> {ex.input}
                  </div>
                  <div className="text-zinc-200">
                    <span className="text-zinc-400">Output:</span> {ex.output}
                  </div>
                  {ex.explanation && (
                    <div className="text-zinc-400 font-sans text-xs pt-1 border-t border-zinc-800/60 mt-1">
                      <span className="font-medium text-zinc-300">Explanation:</span> {ex.explanation}
                    </div>
                  )}
                </div>
              ))}
        ...
        {problem.constraints && problem.constraints.length > 0 && (
          <div className="pt-2">
            ...
            <ul className="list-disc list-inside text-xs font-mono text-zinc-300 space-y-1">
        ...
        <div className="rounded-xl border border-zinc-800/80 bg-zinc-900/40 p-4 space-y-2">
          ...
          <div className="flex flex-wrap gap-1.5 pt-1">
            {problem.keyConcepts.map((concept, i) => (
              <span
                key={i}
                className="inline-flex items-center rounded-md bg-zinc-800 px-2 py-1 text-xs font-medium text-zinc-200 border border-zinc-700/50"
              >
                {concept}
              </span>
            ))}
      ...
      <section className="rounded-xl border border-zinc-800/80 bg-zinc-900/40 p-5 space-y-3">
      ...
      {problem.dryRun && (
        <section className="rounded-xl border border-zinc-800/80 bg-zinc-900/40 p-5 space-y-3">
          ...
          <div className="rounded-lg border border-zinc-800 bg-[#0d0d11] p-3.5 text-xs font-mono text-zinc-300 overflow-x-auto whitespace-pre leading-relaxed">
            {problem.dryRun}
          </div>
        </section>
      )}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div className="rounded-lg border border-zinc-800 bg-zinc-900/50 p-4 space-y-1">
          ...
          <p className="text-xs font-mono text-zinc-200 pt-0.5">
